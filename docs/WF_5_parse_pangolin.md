...
### Runs and analyzes the `all_date_num.fasta` file using the pangolin tool, and pushes the results to the database.

<br />
<br />

## How to use the Workflow 5 Script:

<br />

 - **Step 1**
   - Start the main script, and type `5` to enter the fifth step of the daily workflow script.
   - Alternatively, this script is called as the fifth step to the `start` command.
...
<br />
 ...
 - **Step 2**
   ...
   - *If the user has specified 'cli' as the `analysis pathway` in `private_cache.json`:*
     ...
     - If this script is called by typing `5` at the start of the daily workflow script, the user must specify the `.fasta` file they would like to be analyzed.  Once selected the program proceeds to **Step 3**.
...
<br />
 ...
 - **Step 4**
   ...
   - *If the `analysis pathway` key is set to `online`:*
     -  The script now prompts the user to select the `.csv` results file from pangolin to analyze.
   - Finally, the script formats the `.csv` file and saves the information to the main SQL database
...
- **run_pangolin()** - (`cli` only)
  - Send commands to the linux server to run the local pangolin analysis, and output the results to a specified location

<br />

- **receive_pangolin_df()** - (`cli` only)
  - Pull the results file from the step above back into the parent folder with the name `results.csv`
...
- **get_pango_dfs(nc_path)**
  - If the `analysis pathway` is 'online', prompt the user to select the pangolin results file.
    - If the `analysis pathway` is 'cli', then the script already knows the location and name of the pangolin results file.
